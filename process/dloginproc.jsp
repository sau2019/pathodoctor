...
<%
    ...
    rs = st.executeQuery("select * from dregister where email ='" + email + "'  and password='"+md5_encoded_pass+"'");
	 if (rs.next()) {

	 String dname=rs.getString("name");
     int duserid= rs.getInt("id");
    session.setAttribute("duserid", duserid);
    session.setAttribute("dname",dname);
		//out.println("welcome " + userid);
       // out.println("<a href='logout.jsp'>Log out</a>");
        
    response.sendRedirect("../doctor/doctordash.jsp");
    } 
    else{
        response.sendRedirect("../dloginerror.jsp");
    }
